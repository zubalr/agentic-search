...
async def main():
    ...
    if args.source in ['google', 'both']:
        ...
        for i, query in enumerate(queries):
            try:
                logger.info(f"[Google Places] Fetching for query {i+1}/{len(queries)}: {query.keyword} (lat={query.lat}, lng={query.lng})")
                from scripts.fetch_google_places import fetch_all_places_info
                result = fetch_all_places_info(google_client, query, "*")
                logger.info(f"[Google Places] Success for query {i+1}/{len(queries)}: {query.keyword}")
